...
import { ApplyleaveaddComponent } from '../applyleaveadd/applyleaveadd.component'
...
export class ApplyleavesComponent implements OnInit {
  ...
  constructor(
    ...
    this.dataSource.store = new CustomStore({
      load: (loadOptions) => {
        ...
        let that = this;
        return that.leaveService.searchPage(this.searchModel)
          .toPromise()
          .then(response => {
            return {
    ...
    this.leaveService.getById(currentuser.employeeId, lvNo).subscribe(
      data => {
        ...
        if (result.leaveStatus === 'DRAF') {
          ...
          this.bsModalRef = this.modalService.show(ApplyleaveaddComponent, Object.assign({}, { class: 'modal-lg',ignoreBackdropClick: true, initialState }));
  ...
  openModalWithComponent() {
    ...
    this.bsModalRef = this.modalService.show(ApplyleaveaddComponent, { class: 'modal-lg',ignoreBackdropClick: true});
