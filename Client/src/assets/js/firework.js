...
function randomColor(options) {
  ...
  if (notSame) {
    while (color === lastColor) {if (window.CP.shouldStopExecution(0)) break;
      color = randomColorSimple();
    }window.CP.exitedLoop(0);
  } else
  if (notColor) {
    while (color === notColor) {if (window.CP.shouldStopExecution(1)) break;
      color = randomColorSimple();
...
function randomFastShell() {
  ...
  if (isRandom) {
    while (fastShellBlacklist.includes(shellName)) {if (window.CP.shouldStopExecution(2)) break;
...
function seqPyramid() {
  ...
  let count = 0;
  let delay = 0;
  while (count <= barrageCountHalf) {if (window.CP.shouldStopExecution(3)) break;
...
function seqSmallBarrage() {
  ...
  let count = 0;
  let delay = 0;
  while (count < barrageCount) {if (window.CP.shouldStopExecution(4)) break;
    if (count === 0) {
...
function update(frameTime, lag) {
  ...
  COLOR_CODES_W_INVIS.forEach(color => {
    ...
    for (let i = stars.length - 1; i >= 0; i = i - 1) {if (window.CP.shouldStopExecution(5)) break;
      const star = stars[i];
      ...
      if (star.life <= 0) {
        ...
        if (star.sparkFreq) {
          ...
          while (star.sparkTimer < 0) {if (window.CP.shouldStopExecution(6)) break;
    ...
    for (let i = sparks.length - 1; i >= 0; i = i - 1) {if (window.CP.shouldStopExecution(7)) break;
...
function render(speed) {
  ...
  while (BurstFlash.active.length) {if (window.CP.shouldStopExecution(8)) break;
...
function createParticleArc(start, arcLength, count, randomness, particleFactory) {
  ...
  if (end > start) {
    ...
    for (let angle = start; angle < end; angle = angle + angleDelta) {if (window.CP.shouldStopExecution(9)) break;
      particleFactory(angle + Math.random() * angleDelta * randomness);
    }window.CP.exitedLoop(9);
  } else {
    for (let angle = start; angle > end; angle = angle + angleDelta) {if (window.CP.shouldStopExecution(10)) break;
      particleFactory(angle + Math.random() * angleDelta * randomness);
...
function createBurst(count, particleFactory, startAngle = 0, arcLength = PI_2) {
  ...
  for (let i = 0; i <= C_HALF; i++) {if (window.CP.shouldStopExecution(11)) break;
    ...
    for (let i = 0; i < partsPerArc; i++) {if (window.CP.shouldStopExecution(12)) break;
...
class Shell {
  ...
  burst(x, y) {
    ...
    if (this.glitter === 'medium') {
      sparkFreq = 200;
      sparkSpeed = 0.44;
      sparkLife = 700;
      sparkLifeVariation = 2;
    } else
    if (this.glitter === 'heavy') {
      sparkFreq = 80;
      sparkSpeed = 0.8;
      sparkLife = 1400;
      sparkLifeVariation = 2;
    } else
    if (this.glitter === 'thick') {
      sparkFreq = 16;
      sparkSpeed = isHighQuality ? 1.65 : 1.5;
      sparkLife = 1400;
      sparkLifeVariation = 3;
    } else
    if (this.glitter === 'streamer') {
      sparkFreq = 32;
      sparkSpeed = 1.05;
      sparkLife = 620;
      sparkLifeVariation = 2;
